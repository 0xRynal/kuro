...
module.exports = {
    ...
    execute(message) {
        ...
        if (message.mentions.users.has(message.client.user?.id) && !message.content.startsWith(config.prefix)) {
            ...
            if (helpCmd) { try { helpCmd.execute(message, []); } catch (e) { console.error('gestion', e); } return; }
        }
        if (!message.content.startsWith(config.prefix)) return;
        const args = message.content.slice(config.prefix.length).trim().split(/ +/);
        const cmd = message.client.commands.get(args.shift().toLowerCase());
        if (!cmd) return;
        try { cmd.execute(message, args); } catch (e) { console.error('gestion', e); message.reply('❌ Erreur.').catch(() => {}); }
    },
};
