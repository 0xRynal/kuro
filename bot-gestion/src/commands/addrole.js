...
module.exports = { data: { name: 'addrole' }, async execute(message, args) {
    ...
    if (target.roles.cache.has(role.id)) return message.reply(`❌ ${target} a déjà le rôle ${role}.`);

    await target.roles.add(role);
    await message.reply(`✅ Rôle ${role} ajouté à ${target}.`);

    const logChId = config.getConfig(message.guild.id).logChannelId;
    if (logChId) {
        const ch = message.guild.channels.cache.get(logChId);
        if (ch) ch.send({ embeds: [{ color: 0x5865F2, title: '➕ Addrole', fields: [
