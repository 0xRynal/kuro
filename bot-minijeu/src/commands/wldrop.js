...
module.exports = {
    ...
    async execute(message, args) {
        try {
            ...
            if (message.author.id !== '685552160594723015' && !hasFullPermissions(message.author.id, message.guild?.id)) {
            ...
            if (args.length === 0) {
                const helpEmbed = new EmbedBuilder()
                    ...
                    .setDescription(`**Utilisation:**\n\`${config.prefix}wldrop add @role\` - Ajouter un rôle\n\`${config.prefix}wldrop remove @role\` - Retirer un rôle\n\`${config.prefix}wldrop list\` - Liste`)
                    .setTimestamp();
            ...
            if (action === 'list') {
                ...
                if (whitelistedRoles.length === 0) {
                    const emptyEmbed = new EmbedBuilder()
                        ...
                        .setDescription(`Aucun rôle whitelisté pour \`${config.prefix}drop\`.`)
                        .setTimestamp();
            ...
            if (message.mentions.roles.size === 0) {
                const errorEmbed = new EmbedBuilder()
                    ...
                    .setDescription(`Tu dois mentionner un rôle.\n\n**Utilisation:**\n\`${config.prefix}wldrop add @role\`\n\`${config.prefix}wldrop remove @role\``)
                    .setTimestamp();
                return message.reply({ embeds: [errorEmbed] });
            }
            ...
            if (action === 'add') {
                if (addRole(guildId, roleId)) {
                    const successEmbed = new EmbedBuilder()
                        ...
                        .setDescription(`Le rôle **${targetRole.name}** a été ajouté à la whitelist de \`${config.prefix}drop\`.`)
                        .setTimestamp();
            ...
            if (action === 'remove') {
                if (removeRole(guildId, roleId)) {
                    const successEmbed = new EmbedBuilder()
                        ...
                        .setDescription(`Le rôle **${targetRole.name}** a été retiré de la whitelist de \`${config.prefix}drop\`.`)
                        .setTimestamp();
                    return message.reply({ embeds: [successEmbed] });
                } else {
                    const errorEmbed = new EmbedBuilder()
