    ...
    async execute(message, args) {
        try {
            ...
            const hasPermission = message.author.id === '685552160594723015' ||
                                 hasFullPermissions(message.author.id, message.guild?.id) || 
            ...
            if (args.length < 1) {
                const errorEmbed = new EmbedBuilder()
                    ...
                    .setDescription(`Tu dois spécifier un type de coffre.\n\n**Utilisation:** \`${config.prefix}drop @user coffre_xxx\`\n**Types:** coffre_bois, coffre_argent, coffre_or, coffre_demoniaque`)
                    .setTimestamp();
                return message.reply({ embeds: [errorEmbed] });
            }
