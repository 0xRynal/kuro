...
module.exports = {
    ...
    async execute(message, args) {
        try {
            if (args.length < 1) {
                return message.reply(`❌ Usage: \`${config.prefix}ameliorer <index_objet>\`\nUtilise \`${config.prefix}inventaire\` pour voir les index.`);
            }
            ...
            if (!user.items || itemIndex < 0 || itemIndex >= user.items.length) {
                return message.reply({
                    embeds: [{
                        ...
                        description: `Utilise \`${config.prefix}inventaire\` pour voir tes objets et leurs index.`,
            ...
            // Retirer les pièces
            user.coins = Math.max(0, user.coins - upgradeCost);
