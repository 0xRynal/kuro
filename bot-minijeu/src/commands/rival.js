    ...
    async execute(message, args) {
        try {
            ...
            if (args[0] === 'list') {
                ...
                if (rivals.length === 0) {
                    const errorEmbed = new EmbedBuilder()
                        ...
                        .setDescription(`Tu n'as pas encore de rivaux.\n\nUtilise \`${config.prefix}arene @user\` pour créer une rivalité !`)
                        .setFooter({ 
                    ...
                    setTimeout(async () => {
                        ...
                        try {
                            ...
                            const fakeMessage = Object.assign({}, message, {
                                content: `${config.prefix}arene <@${challengerUser.id}>`,
                                mentions: {
                                    users: mentionsUsers,
                            ...
                            const errorEmbed = new EmbedBuilder()
                                .setColor(0xFF0000)
                                .setTitle('❌ Erreur')
                                .setDescription(`Erreur lors du lancement.\n\nUtilisez: \`${config.prefix}arene @${challengerUser.username}\`\n\nErreur: ${error.message}`)
                                .setTimestamp();
            ...
            const helpEmbed = new EmbedBuilder()
                ...
                .setDescription(`**Commandes:**\n\n\`${config.prefix}rival list\` - Liste tes rivaux\n\`${config.prefix}rival challenge @user <mise>\` - Défie un joueur\n\`${config.prefix}rival accept\` - Liste tes défis\n\`${config.prefix}rival accept @user\` - Accepte un défi`)
                .addFields({
                    name: '💡 Astuce',
