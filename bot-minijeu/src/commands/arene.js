    ...
    data: {
        ...
        async execute(message, args) {
            ...
            while (userHP > 0 && opponentHP > 0 && turn < 20) {
                ...
                await combatMsg.edit({ embeds: [combatEmbed] }).catch(err => {
                    if (err.code !== 10008) {
                        console.error('Erreur lors de l\'édition du message combat:', err);
                    }
                });
            ...
            if (userWon) {
                ...
                await combatMsg.edit({ embeds: [victoryEmbed] }).catch(err => {
                    if (err.code !== 10008) {
                        console.error('Erreur lors de l\'édition du message victoire:', err);
                    }
                });
            } else {
                // Handle challenge bet if exists
                ...
                await combatMsg.edit({ embeds: [defeatEmbed] }).catch(err => {
                    if (err.code !== 10008) {
                        console.error('Erreur lors de l\'édition du message défaite:', err);
                    }
                });
            }
        } catch (error) {
